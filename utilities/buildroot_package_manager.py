...
import re
import fileinput
from houdini_config import RUNC_VERSION, DOCKER_ENGINE_VERSION
from path_manager import RUNC_MAKEFILE_PATH, DOCKER_ENGINE_MAKEFILE_PATH, FILESYSTEM_BUILD
...
class BuildrootPackageManager:
    ...
    def update_docker_engine_version():
        if not DOCKER_ENGINE_MAKEFILE_PATH.exists():
            print(f"Error: Docker Engine {DOCKER_ENGINE_MAKEFILE_PATH} not found.")
            sys.exit(0)

        content = DOCKER_ENGINE_MAKEFILE_PATH.read_text()
        
        new_content, count = re.subn(
            r"^(DOCKER_ENGINE_VERSION\s*=\s*)[\d.]+",
            lambda m: m.group(1) + DOCKER_ENGINE_VERSION,
            content,
            flags=re.MULTILINE
        )

        if count == 0:
            print("No DOCKER_ENGINE_VERSION line found.")
        else:
            DOCKER_ENGINE_MAKEFILE_PATH.write_text(new_content)
            print(f"✅ Updated DOCKER_ENGINE_VERSION to {DOCKER_ENGINE_VERSION} in {DOCKER_ENGINE_MAKEFILE_PATH}")